    ...
    <div className="space-y-10 px-6 lg:px-10">
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-6">
        {images.map((img) => (
          <div
            key={img.id}
            className="rounded-lg overflow-hidden border border-gray-300 shadow-lg relative transform hover:scale-105 transition-transform duration-300 p-3"
          >
            <img
              src={img.img}
              alt="image"
            ...
            <div className="absolute inset-0 flex items-center justify-center opacity-0 hover:opacity-100 transition-opacity duration-300">
              <div className="bg-black bg-opacity-50 absolute inset-0 flex flex-col gap-4 items-center justify-center px-4 pb-5">
                <p className="text-white text-xl text-center font-semibold">{img.title}</p>
                <button className="btn btn-info btn-sm text-white" onClick={navigate("/allToys")}>See More</button>
              </div>
            </div>
          </div>
        ))}
